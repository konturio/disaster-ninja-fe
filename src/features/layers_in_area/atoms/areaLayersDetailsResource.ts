...
export const areaLayersDetailsParamsAtom = createAtom(
  ...
  (
    ...
    // compare layers and event id with previous request to avoid extra requests
    // Why? Because some enabled layers aren't from areaLayersResource and do NOT require
    // to perform additional details request
    // Yet same layers for different events DO require new details request
    if (
      state &&

...
export function CreateLayerFieldContainer( { data, id, index, onReorder, onRemove, className }: CreateLayerFieldContainerProps ) {
  ...
  const updateAtomName = useCallback((ev: ChangeEvent<HTMLInputElement>) => {
    updateName(ev.target.value);
  }, [updateName]);

  const updateAtomType = useCallback((ev: ChangeEvent<HTMLSelectElement>) => {
  ...
  return (
    ...
    >
      ...
      <div className={s.fieldPlaceholder}>
        ...
        <Input className={s.input} onChange={updateAtomName} value={atomState.name} />
      </div>
      <div className={s.fieldPlaceholder}>
        <div className={s.fieldLabel}>
          {i18n.t('Type')}

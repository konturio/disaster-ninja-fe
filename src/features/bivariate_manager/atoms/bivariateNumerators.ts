...
const extractAvailableNumeratorsWithDenominators = (stat: Stat) => {
  ...
  correlationRates.forEach(({ x: { quotient: [xNumerator, xDenominator], }, y: { quotient: [yNumerator, yDenominator], }, }) => {
    if (!x.find(i => i.numeratorId === xNumerator && i.denominators[0] === xDenominator)) {
      x.push({ numeratorId: xNumerator, denominators: [xDenominator], selectedDenominator: xDenominator });
    }

    if (!y.find(i => i.numeratorId === yNumerator && i.denominators[0] === yDenominator)) {
      y.push({ numeratorId: yNumerator, denominators: [yDenominator], selectedDenominator: yDenominator });
    }
  });

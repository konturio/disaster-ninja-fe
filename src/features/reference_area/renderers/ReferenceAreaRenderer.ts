...
export class ReferenceAreaRenderer extends LogicalLayerDefaultRenderer {
  ...
  async updateOrSetSource({
    ...
    if (stateSource.type !== 'geojson') {
      ...
      throw Error('Reference area must be geojson');
    }
    ...
    if (
      ...
      this.removeSourcesAndLayers({ map });
      throw Error('Reference area must be Feature or FeatureCollection');
    }

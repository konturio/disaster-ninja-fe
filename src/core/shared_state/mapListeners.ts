...
type MapListener = (event: mapLibre.MapMouseEvent & mapLibre.EventData, map?: mapLibre.Map) => boolean;
...
export function registerMapListener(
  eventType: MapEvent, listener: MapListener, priority1to100: number = 50
): () => void {
...
export const mapListenersAtom = createBindAtom(
  ...
  ({ onAction }, state: MapListenersAtomState = defaultListeners) => {
    onAction('addMapListener', ({ eventType, listener, priority }) => {
      ...
      if (!listenerCategory.length) listenerCategory.push({ listener, priority })
      ...
      else for (let i = 0; i < listenerCategory.length; i++) {
        ...
        if (priority < listenerWrap.priority) {
          ...
          break
        } else if (priority > listenerWrap.priority && priority < listenerCategory[i + 1].priority) {
          listenerCategory.splice(i + 1, 0, { listener, priority })
          break
        }
      }
